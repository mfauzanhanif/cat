    <div class="space-y-6">
        ...
        @if($mySubjects->count() > 0)
            <flux:card>
                <flux:heading size="lg" class="mb-3">Mata Pelajaran Saya</flux:heading>
                <div class="flex flex-wrap gap-2">
                    @foreach($mySubjects as $subject)
                        <flux:badge color="purple" size="sm">{{ $subject->name }} ({{ $subject->code }})</flux:badge>
                    @endforeach
                </div>
            </flux:card>
        @else
        ...
        <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <flux:card class="flex items-center gap-4">
                ...
                <div>
                    <flux:text class="text-sm text-zinc-500">Total Soal</flux:text>
            ...
            <flux:card class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <flux:heading size="xl">{{ $totalParticipants }}</flux:heading>
                </div>
            </flux:card>
        </div>
