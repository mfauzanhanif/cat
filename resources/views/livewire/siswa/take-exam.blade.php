    <div class="space-y-6" x-data="examTimer({{ $remainingSeconds }})" x-init="startTimer()">
        ...
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div class="flex items-center gap-3 rounded-lg bg-zinc-100 px-4 py-2 dark:bg-zinc-800">
                <flux:icon.clock class="h-5 w-5" x-bind:class="remainingSeconds < 300 ? 'text-red-500 animate-pulse' : ''" />
                <span
                    class="font-mono text-lg font-bold"
                    x-bind:class="remainingSeconds < 300 ? 'text-red-500' : ''"
                ...
                @if(count($questions) > 0 && isset($questions[$currentQuestionIndex]))
                    ...
                    <flux:card>
                        <div class="mb-4 flex items-center justify-between">
                            <flux:badge color="purple">Soal {{ $currentQuestionIndex + 1 }} dari {{ count($questions) }}</flux:badge>
                        </div>
                        ...
                        <div class="space-y-3">
                            @foreach(['A' => 'option_a', 'B' => 'option_b', 'C' => 'option_c', 'D' => 'option_d'] as $letter => $optionKey)
                                <button
                                    wire:click="selectAnswer({{ $currentQuestion['answer_id'] }}, '{{ $letter }}')"
                                    class="flex w-full items-start gap-3 rounded-lg border p-4 text-left transition-all hover:bg-zinc-50 dark:hover:bg-zinc-800
